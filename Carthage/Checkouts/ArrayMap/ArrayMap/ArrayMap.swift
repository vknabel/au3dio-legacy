...
public struct ArrayMap<Key: Hashable, Value>: SequenceType {
    ...
    public func dropFirst(_ n: Int = 1) -> SubSequence {
        return Array<Generator.Element>(self).dropFirst(n)
    }
    
    /// - Requires: `n >= 0`
    /// - Complexity: O(`self.count`)
    public func dropLast(_ n: Int = 1) -> SubSequence {
        return Array<Generator.Element>(self).dropLast(n)
    }
    
    /// - Requires: `maxLength >= 0`
    public func prefix(_ maxLength: Int) -> SubSequence {
        return Array<Generator.Element>(self).prefix(maxLength)
    }
    
    /// - Requires: `maxLength >= 0`
    public func suffix(_ maxLength: Int) -> SubSequence {
        return Array<Generator.Element>(self).suffix(maxLength)
    }
    
    /// - Requires: `maxSplit >= 0`
    public func split(maxSplits: Int, allowEmptySlices: Bool, @noescape isSeparator: (Generator.Element) throws -> Bool) rethrows -> [SubSequence] {
        return try Array<Generator.Element>(self).split(maxSplits, allowEmptySlices: allowEmptySlices, isSeparator: isSeparator)
    }
}
